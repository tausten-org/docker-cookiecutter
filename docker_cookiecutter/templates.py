...
def encode_template_sources(sources):
    ...
    for t in sources:
        template = t.template
        if t.checkout: template += CONST.COMPONENT_DELIM + CONST.CHECKOUT_PREFIX + t.checkout
        if t.directory: template += CONST.COMPONENT_DELIM + CONST.DIRECTORY_PREFIX + t.directory
